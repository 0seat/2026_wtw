...
주의 — **로봇은 스텝을 밟긴 했다**(접지 비율 0.25~0.57). 몸통이 0.14~0.28로 크게 바운스하다
...
|---|---|---|---|---|---|---|---|
| 0 | `vx` | 2.0 | m/s | 전후 선속도 | `1.0`/`0.0` | 커리큘럼 도달 **±2.38** (yaw와 결합, §4.3) | 사용 |
| 1 | `vy` | 2.0 | m/s | 좌우 게걸음 | `0.0` | `[-0.6, 0.6]` 전 구간 (bin 1개 = 균등) | 사용 |
| 2 | `yaw_v` | 0.25 | rad/s | 제자리 회전 | `0.0` | 도달 **[-3.33, +2.86]** (vx와 결합) | 사용 |
| 3 | `height` | 2.0 | m | 몸통 높이 추종 | `0.0` | `[-0.25, 0.15]` 전 구간 | 사용 |
| 4 | `step_freq` | 1.0 | Hz | 발걸음 빈도 | `3.0` | `[2.0, 4.0]` 전 구간 | 사용 |
| 5 | `gait_phase` | 1.0 | rad | 걸음 위상차 | **`0.5`** | 주의 — **`{0, 0.5}` 양자화** (§5.2) | 이산 |
| 6 | `gait_offset` | 1.0 | rad | 걸음 오프셋 | **`0.0`** | 주의 — **`{0, 0.5}` 양자화** | 이산 |
| 7 | `gait_bound` | 1.0 | rad | 걸음 바운드 | **`0.0`** | 주의 — **`{0, 0.5}` 양자화** | 이산 |
| 8 | `duty` (duration) | 1.0 | ratio | 체공/지지 비율 | `0.5` | **`[0.5, 0.5]` 상수 — 조작 불가** | 불가 |
| 9 | `footswing` (`foot_height`) | 0.15 | m | 발 들어올림 높이 | `0.08` | `[0.03, 0.35]` 전 구간 | 사용 |
| 10 | `pitch` | 0.3 | rad | 몸통 피치 | `0.0` | `[-0.4, 0.4]` 전 구간 | 사용 |
| 11 | `roll` | 0.3 | rad | 몸통 롤 | `0.0` | **`[-0.0, 0.0]` 상수 — 조작 불가** | 불가 |
| 12 | `stance_width` | 1.0 | m | 다리 벌림 너비 | `0.25` | `[0.10, 0.45]` 전 구간 | 사용 |
| 13 | `stance_length` | 1.0 | m | 다리 앞뒤 길이 | `0.45` | `[0.35, 0.45]` (고정 사용 0.45) | 고정 |
| 14 | `aux` | 1.0 | - | 보조(compliance) | `0.0` | — `[0, 0.01]` (사용 안 함) | 고정 |
...
주의 — **고정 5차원의 이유가 두 종류라는 점에 유의.** `duty`·`roll`은 *학습되지 않아서*
...
> 주의 — **actuator_net을 PD로 근사한다** (= sim-to-sim 격차 ①). 다만 실측해보니 격차가 작다 —
...
- 주의 — **solver 설정의 단일 출처는 `policy.create_env`다.** 한때 `check.build()`가
...
- **HFIELD·PLANE 쌍은 컬링을 건너뛴다**(MJX 내부 `_MAX_NCON=8`) — 바닥 접촉은
  영향받지 않는다.
- 주의 — **근사다.** k가 실제 동시 접촉 쌍보다 작으면 접촉이 조용히 누락되어 벽을
...
주의 — 속도는 **반드시 몸통 좌표계로** 잰다(`check.body_velocity`, `mju_quat2Mat` 대조 오차
