...
주의 — **`<include>`로 만들지 말 것.** menagerie `go1.xml`은 `meshdir="assets"`(상대경로)를
...
PLATFORM_T = 0.10
#: 주의 — **넉넉해야 한다.** LLC는 직진 명령에도 완만히 선회한다 — vx=0.8에서 y가
...
def set_broadphase(spec, max_geom_pairs: int = 0, max_contact_points: int = 0) -> int:
    ...
    주의 — **컴파일 전 spec에 넣어야 한다.** 컴파일된 `mj_model`의 `numeric_data`는
    크기가 고정이라 나중에 추가할 수 없다.

    주의 — mujoco 버전에 따라 `add_numeric` 시그니처가 다를 수 있다. 조용히 넘어가면
...
def broadphase_report(mj_model) -> dict:
    ...
    # 주의 — `None`은 **설정을 0으로 둬서 안 넣은 것**과 구분되지 않는다. 여기서는
...
#:
#: 주의 — **3을 쓰면 안 된다.** menagerie Go1은 자기 충돌 geom 42개에 group 3을 쓴다
...
#: 주의 — 착지 발판 길이. **넉넉해야 한다.** 4 m로 뒀다가 vx=1.4 조합이 8초에
...
#: (`PLATFORM_W` 주석의 그 사고).
#:
#: 주의 — 2026-08-06에 로봇이 요철을 벗어나 **옆의 평지를 밟고 지나가는 것**이 영상에서
...
#: 좌우 레인 수. ★ **반드시 짝수여야 한다.**
#:
#: 주의 — 처음에 3(좌·중·우)으로 뒀다가 검증에서 걸렸다 (2026-08-06). 홀수면 가운데
...
def _rough_patch(spec, name: str, x0: float, x1: float, z: float,
    ...
    **디딜 곳의 높이를 모르는 채로 걷기.**
    ...
    주의 — `amp`는 진폭이므로 **이웃 타일 사이의 단차는 최대 `2*amp`**다. `ledge`
...
def rough(amp: float, x0: float = 3.0, run_out: float = RUN_OUT,
    ...
    주의 — 라이다는 이 지형을 **원리적으로 보지 못한다** (광선이 몸통 높이 0.34 m
...
#: 주의 — 예전에는 장애물 **시작** x + 0.8이었다 (2026-08-02 수정). gap은 폭이 최대
...
def ladder(kind: str, values, x0: float = 3.0, spacing: float = 3.0,
    ...
    주의 — 지형 종류는 섞지 않는다. 섞으면 "어디서 막혔나"가 "무엇 때문에 막혔나"를
    ...
    for i, v in enumerate(values):
        ...
        if kind == "gap":
            ...
            # 주의 — 프로파일(`prof_z`)에는 **명목 높이 z**를 넣는다. 요철은 ±amp로
...
def beam(width: float, x0: float = 3.0, run_out: float = RUN_OUT):
    ...
    주의 — 여기서는 **옆으로 떨어지는 것이 곧 지형 실패**다. 다른 측정에서는
    ...
    **`scripted.foot_track()`으로 평지에서 둘을 따로 재고**, 이 지형은 그
    ...
    주의 — 옛 주석의 "**측정 중에는 표류를 보정하지 않는다**(열린 루프). 보정을 넣으면
...
#: 주의 — 여기를 넓히면 "다리 폭"이 아니라 "직진 능력"을 재게 된다.
#: 주의 — 좁히면 회복 구간에서 떨어지는 것이 다리 실패로 집계된다.
