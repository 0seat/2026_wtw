...
class ActionConfig:
    ...
    #: ⚠️ 2026-08-02에 ±0.4로 넓혔다가 **되돌렸다.** 넓힌 근거는 "경사 스윕에서
    ...
    #:
    #: ⚠️ **이것이 없으면 진로 유지기가 원리적으로 직진하지 못한다.**
...
class CourseConfig:
    ...
    #: ⚠️ **정책의 실제 속도로 타임아웃 안에 도달 가능해야 한다.** 2026-07-29:
...
class MazeConfig:
    ...
    ⚠️ 미로 하나는 MJX 모델 하나다 — env마다 다른 레이아웃을 줄 수 없다
...
class TerrainConfig:
    ...
    ⚠️ **손 스윕으로 한계를 재려던 시도는 폐기됐다** (2026-07-29). 조작 축이 7개라
    조합이 폭발하고(4^7≈16k), 손으로 고른 조합은 "두 축이 동시에 있어야 넘는"
    종속성을 원리적으로 못 찾는다. 여기서는 PPO가 8축을 동시에 굴려 그 탐색을 한다.

    ⚠️ 종류를 섞지 말 것. 섞으면 "어디서 막혔나"가 "무엇 때문에 막혔나"를 못 준다.
    """
    ...
    #:
    #:   ⚠️ 5°/10°에서 대조군 A가 죽으면 ①의 등가성 논증이 틀린 것이다. 그때
    #:      의심할 것은 경사 자체가 아니라 **램프 진입/이탈 모서리**다(중력
    #:      랜덤화에는 모서리가 없다).
    PRESETS = {
        "gap":    (0.05, 0.10, 0.15, 0.20, 0.25, 0.30),
        #: 턱 각 단의 **증분** (누적 0.55 m). ⚠️ 2026-08-09에 재설계했다.
        ...
        #: 외나무다리 **전폭** m. ⚠️ 이 값은 `scripted.foot_track()`의 실측
        ...
        #: 요철 **진폭** ±m. ⚠️ 이웃 타일 단차는 최대 **2배**다 (`modules._rough_patch`).
        ...
        #: ⚠️ **첫 사다리 (0.02~0.07)는 너무 쉬웠다** (2026-08-06). 대조군 A가
...
@dataclasses.dataclass(frozen=True)
class RewardConfig:
    #: ⚠️ 이 셋의 **상대 크기**가 정책 성격을 정한다. 2026-07-29 실패 기록:
...
class TerminationConfig:
    ...
    #: P1 평지 20 m 기준. ⚠️ 20 s로 두면 안 된다 — 수동 기준 제어기(vx 1.0)가
...
@dataclasses.dataclass(frozen=True)
class PPOConfig:
    #: ⚠️ 이 값은 **HLC 스텝**을 센다. HLC 1스텝 = LLC 5 × 물리 4 = **물리 20스텝**이므로
    ...
    #: ⚠️ **P1은 진단용 짧은 실행이다** (2026-07-29 결정). P1 정책은 P2로 이어붙일
    ...
    num_envs: int = 2048
    #: ⚠️ brax는 `batch_size × unroll_length × num_minibatches` 만큼의 env 스텝을
    ...
    #:
    #: ⚠️ **`unroll_length`로 갱신 횟수를 벌지 말 것.** brax GAE는 `unroll_length`
    ...
    #: ⚠️ 1e-2는 **너무 크다.** 실패한 실행에서 entropy_loss(-0.0536)가
    ...
    #: ⚠️ **`num_evals`는 예산을 덮어쓴다.** brax는 eval마다 최소 1회 학습을 하므로
...
class HLCConfig:
    ...
    #:
    #: ⚠️ **근사다.** k가 실제 동시 접촉 쌍보다 작으면 접촉이 조용히 누락되어
    #: 로봇이 벽을 통과한다. 그래서 기본은 **끔**이고, 켤 때는 반드시 `world`와
    #: A/B로 궤적이 유지되는지 확인한다 (`full` -> `world` 전환과 같은 절차).
    #: ⚠️ HFIELD·PLANE 쌍은 이 컬링을 **건너뛴다**(MJX 내부 `_MAX_NCON=8`).
    #:    즉 바닥 접촉은 영향을 받지 않는다 — 다행이다.
    #: ⚠️ 캡은 **geom 종류 쌍마다** 적용된다(CAPSULE-BOX 따로, BOX-BOX 따로).
...
#: ⚠️ 근사이므로 A/B 게이트를 통과한 값만 쓸 것 (`HLCConfig.max_geom_pairs`).
MAZE_PAIRS = 32
...
def maze_config(n: int = 5, seed: int = 0, cruise: float = 0.7,
    ...
    ⚠️ 노트북에서 손으로 계산하지 말 것. 슬로프 실행의 상수(60 s / reach 110)를
    ...
    ⚠️ `corner_slack`은 1.3에서 **1.5로 올렸다** (2026-08-10). 1.3(5×5에서 21 s)로
    ...
    ⚠️ 경로 길이를 알려면 미로를 만들어야 하는데(BFS), 모델까지 컴파일하면
    ...
    # ⚠️ **`num_timesteps`를 갱신 단위의 배수로 맞춘다.** brax는 eval 구간마다
